...
module ExercisesDay2_3Clock =
    ...
    let create (hours: int) (minutes: int) : Clock =
        let totalMinutes = ((hours + 24) * 60 + minutes) % 1440
        if totalMinutes < 0 then
            { hours = (totalMinutes + 1440) / 60; minutes = (totalMinutes + 1440) % 60 }
        else
    ...
    let subtract (minutes: int) (clock: Clock) =
        let totalMinutes = (clock.hours * 60 + clock.minutes - minutes + 1440) % 1440
        { hours = totalMinutes / 60; minutes = totalMinutes % 60 }
