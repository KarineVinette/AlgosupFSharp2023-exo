...
module ExercisesDay2_4RobotSimulator =
    ...
    let move instructions robot =
            instructions |> Seq.fold (fun r i -> match i with
                                           | 'R' -> match r.direction with
                                                      | North -> {r with direction = East}
                                                      | East -> {r with direction = South}
                                                      | South -> {r with direction = West}
                                                      | West -> {r with direction = North}
                                           | 'L' -> match r.direction with
                                                      | North -> {r with direction = West}
                                                      | West -> {r with direction = South}
                                                      | South -> {r with direction = East}
                                                      | East -> {r with direction = North}
                                           | 'A' -> match r.direction with
                                                      | North -> {r with position = fst r.position, snd r.position + 1}
                                                      | East -> {r with position = fst r.position + 1, snd r.position}
                                                      | South -> {r with position = fst r.position, snd r.position - 1}
                                                      | West -> {r with position = fst r.position - 1, snd r.position}
                                           | _ -> failwith "Invalid instruction") robot
