...
module ExercisesDay1 =
    ...
    let SortingAWordListByLength() =
        ...
        let result = initialValue |> List.sortBy (fun word -> word.Length)
    ...
    let SumTheIntegersContainedInTheStrings() =
        ...
        let result = initialValue |> List.map int |> List.sum
    ...
    let ComputerTheAverageNumberOfLetters() =
        ...
        let result = initialValue |> List.map (fun x ->float x.Length) |> List.average
    ...
    let CaesarCipher() =
        ...
        let encoder (l: list<char>) =
            l |> List.map (fun x -> char (int x + 13))

        let decoder (l: list<char>) =
            l |> List.map (fun x -> char (int x - 13))
    ...
    let TestIfAWordHasNoRepeatingLetter() =
        let test (s: string) =
                s |> Seq.distinct |> Seq.length = s.Length
