...
module ExercisesDay2_5Allergies =
    ...
    let allergicTo codedAllergies allergen =
        let allergenValue = int allergen
        codedAllergies &&& allergenValue = allergenValue

    let list codedAllergies = 
        let rec loop codedAllergies allergens =
            match allergens with
            | [] -> []
            | allergen :: rest ->
                if allergicTo codedAllergies allergen then
                    allergen :: loop codedAllergies rest
                else
                    loop codedAllergies rest
        loop codedAllergies [Allergen.Eggs; Allergen.Peanuts; Allergen.Shellfish; Allergen.Strawberries; Allergen.Tomatoes; Allergen.Chocolate; Allergen.Pollen; Allergen.Cats]
